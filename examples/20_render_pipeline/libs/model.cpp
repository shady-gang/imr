...
Model::Model(const char* path, Device& device) {
    ...
    triangles_gpu = std::make_unique<imr::Buffer>(device, sizeof(Vertex) * triangles.size() * 3, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
    Vertex * vertex_data;
    CHECK_VK(vkMapMemory(device.device, triangles_gpu->memory, triangles_gpu->memory_offset, triangles_gpu->size, 0, (void**) &vertex_data), abort());
    ...
    vkUnmapMemory(device.device, triangles_gpu->memory);
    ...
    if (scene->HasCameras()) {
        ...
        loaded_camera = Camera {
            ...
            .fov = camera->mHorizontalFOV * 180 / M_PI,
        };
