...
int main(int argc, char ** argv) {
    ...
    imr::Image depth_image (device, VK_IMAGE_TYPE_2D, { (uint32_t) width, (uint32_t) height, 1}, depth_format, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
    ...
    while (!glfwWindowShouldClose(window)) {
        swapchain.beginFrame([&](imr::Swapchain::Frame& frame) {
            ...
            //VkSemaphore sem;
            //CHECK_VK(vkCreateSemaphore(device.device, tmpPtr((VkSemaphoreCreateInfo) {
...
//
            ...
            VkRenderingInfoKHR render_info = initializers::rendering_info(
                initializers::rect2D(static_cast<int>(width), static_cast<int>(height), 0, 0),
                1,
            ...
            VkViewport viewport = initializers::viewport(static_cast<float>(width), static_cast<float>(height), 0.0f, 1.0f);
            vkCmdSetViewport(cmdbuf, 0, 1, &viewport);

            VkRect2D scissor = initializers::rect2D(static_cast<int>(width), static_cast<int>(height), 0, 0);
